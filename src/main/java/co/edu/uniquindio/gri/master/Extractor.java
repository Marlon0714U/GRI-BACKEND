...
public class Extractor {
	...
	public Future<Grupo> scrapData(String url, Grupo grupo) {
		...
		int statusConnectionCode = jSoupUtil.getStatusConnectionCode(url);

		if (statusConnectionCode == 200) {

			// Extrae todo el HTML de la url enviada por parametro
			Document document = jSoupUtil.getHtmlDocument(url);
			...
			grupoDAO.save(grupo);
			System.err.println(grupo.getId() + "-" + grupo.getNombre());
	...
	public void invesRepetido(Investigador investigador, String estado) {
		...
		if (!investigador.getIdiomas().isEmpty() || !investigador.getLineasInvestigacion().isEmpty()
